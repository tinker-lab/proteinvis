...
public:
  ...
  void FillTexture(vector<unsigned char> &t, int s, int tx, int ty,
    ...
    for (int x=0; x<size; x++) 
    {
      ...
      Point3f q=(p+Point3f(1,1,1))/2.0*255.0;
        t[k++]= (unsigned char)q[0];
        t[k++]= (unsigned char)q[1];
        t[k++]= (unsigned char)q[2];
...
//      printf("%4d ",map[Index( x , y )]);
  ...
  void Smooth(vector<unsigned char> &t, int s, int tx, int ty){
...
*/      
  ...
  void FillTexture(vector<unsigned char> &texture, const vector<int> &sumtable, 
