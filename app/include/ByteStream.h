...
public:
	...
	double readDouble() {
		if (_readOffset + ByteData::BYTEDATA_SIZEOFDOUBLE > _byteData.getSize()) {
			// TODO: throw some exception
			std::cerr << "tried to read past the end of a ByteStream" << std::endl;
			return 0;
		}
		else {
			double d = ByteData::unpackDouble(&_byteData.getByteArray()[_readOffset]);
			_readOffset += ByteData::BYTEDATA_SIZEOFDOUBLE;
			return d;
		}
	}
	...
	void writeDouble(double d) {
		if (_writeOffset + ByteData::BYTEDATA_SIZEOFDOUBLE > _byteData.getSize()) {
			resize(_byteData.getSize() + ByteData::BYTEDATA_SIZEOFDOUBLE);
		}
		ByteData::packDouble(&_byteData.getByteArray()[_writeOffset], d);
		_writeOffset += ByteData::BYTEDATA_SIZEOFDOUBLE;
	}
