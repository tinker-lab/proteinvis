...
public:
	...
	static const int BYTEDATA_SIZEOFDOUBLE = sizeof(double);
	// TODO: bool
	// TODO: char
	...
	ByteData(double d) {
		_byteArray = new unsigned char [BYTEDATA_SIZEOFDOUBLE];
		_size = BYTEDATA_SIZEOFDOUBLE;
		packDouble(_byteArray, d);
	}
	...
	/// Interprets the *entire* bytedata array as holding a single double
	float toDouble() const {
		if (_size == BYTEDATA_SIZEOFDOUBLE) {
			return unpackDouble(_byteArray);
		}
		else {
			// throw an exception
			return 0.0;
	...
	static void packDouble(unsigned char *bytePtr, double toPack) {
		unsigned char *p = (unsigned char *) &toPack;
		for (int i=0;i<BYTEDATA_SIZEOFDOUBLE;i++) {
			bytePtr[i] = p[BYTEDATA_SIZEOFDOUBLE - i - 1];
		}
	}

	static double unpackDouble(unsigned char *bytePtr) {
		double toReturn = 0;
		unsigned char *p = (unsigned char *) &toReturn;
		for (int i=0;i<BYTEDATA_SIZEOFDOUBLE;i++) {
			p[i] = bytePtr[BYTEDATA_SIZEOFDOUBLE - i - 1];
		}
		return toReturn;
	}
