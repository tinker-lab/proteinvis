...
NewXZRotExperimentHCI::NewXZRotExperimentHCI(MinVR::AbstractCameraRef camera, CFrameMgrRef cFrameMgr, TextureMgrRef texMan, FeedbackRef feedback) : AbstractHCI(cFrameMgr, feedback) {
...
NewXZRotExperimentHCI::~NewXZRotExperimentHCI(){

}

void NewXZRotExperimentHCI::initializeContextSpecificVars(int threadId,MinVR::WindowRef window) {

	initVBO(threadId);
	std::cout<<"NewXZROTExperimentHCI is been called"<<std::endl;
...
void NewXZRotExperimentHCI::closestTouchPair(std::map<int, TouchDataRef> thisRegisteredTouchData, glm::dvec3 &pos1, glm::dvec3 &pos2, double &minDistance) {
...
glm::dvec3 NewXZRotExperimentHCI::convertScreenToRoomCoordinates(glm::dvec2 screenCoords) {
